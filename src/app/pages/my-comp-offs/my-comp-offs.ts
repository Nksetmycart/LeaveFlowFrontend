...
@Component({
  selector: 'app-my-compoff',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './my-comp-offs.html',
  styleUrl: './my-comp-offs.css',
})
export class MyCompOff implements OnInit {
  ...
  ngOnInit(): void {
    this.loadEmployeeCompOffHistory();
  }

  loadEmployeeCompOffHistory(): void {
    ...
    this.leaveService.GetCompOffRequestsByEmployee(employeeId, this.page, this.pageSize).subscribe({
      next: (response: any) => {
        this.apiResponse = response;
        const rawData = response.data || [];
        
        // Normalize session properties cleanly from response payload
        this.compOffHistory = rawData.map((record: any) => ({
          ...record,
        ...
        if (response.totalCount !== undefined) {
          ...
          this.totalItems = this.compOffHistory.length < this.pageSize && this.page === 1 
            ? this.compOffHistory.length 
      ...
      error: (error) => {
        this.isLoading = false;
        console.error("Error retrieving customized comp off logs:", error);
      }
    });
  }
  ...
  get totalPages(): number {
    if (this.totalItems <= this.compOffHistory.length && this.page === 1) return 1;
    return Math.ceil(this.totalItems / this.pageSize) || 1;
  }

  get startItemIndex(): number {
    if (this.compOffHistory.length === 0) return 0;
    return (this.page - 1) * this.pageSize + 1;
  }

  get endItemIndex(): number {
    const computedEnd = this.page * this.pageSize;
    if (this.totalItems <= this.compOffHistory.length && this.page === 1) return this.compOffHistory.length;
  ...
  openCancelConfirmation(compOff: any): void {
    this.compOffToCancel = compOff;
    this.showCancelModal = true;
  }

  closeCancelConfirmation(): void {
    this.showCancelModal = false;
    this.compOffToCancel = null;
    this.isCancelling = false;
  }

  confirmCancel(): void {
    if (!this.compOffToCancel) return;
    ...
    // Note: If backend utilizes CancelCompOff or CancelLeaveRequest, update appropriately. 
    // Here we leverage the designated endpoint or standard cancellation pattern matching service definitions.
    this.leaveService.CancelLeaveRequest(employeeId, this.compOffToCancel.id).subscribe({
      next: (res) => {
  ...
  /**
   * Calculates total comp off days based on date span, startSession, and endSession parameters
   */
  calculateCompOffDays(startDate: any, endDate: any, startSession: string, endSession: string): number {
