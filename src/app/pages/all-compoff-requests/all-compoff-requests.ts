...
@Component({
  selector: 'app-all-compoff-requests',
  standalone: true,
  imports: [CommonModule, RouterLink, FormsModule],
  templateUrl: './all-compoff-requests.html',
  styleUrl: './all-compoff-requests.css',
})
export class AllCompOffRequests implements OnInit {
  ...
  ngOnInit(): void {
    this.loadAllCompanyCompOffRequests();
  }

  loadAllCompanyCompOffRequests(): void {
    this.isLoading = true;
    
    this.leaveService.GetAllCompOffRequests(this.page, this.pageSize).subscribe({
      next: (response: any) => {
        ...
        this.compOffLogs = rawData.map((record: any) => {
          return {
            ...record,
      ...
      error: (error) => {
        this.isLoading = false;
        console.error("Error retrieving global comp off requests statement indexes:", error);
      }
    });
  }
  ...
  get totalPages(): number {
    if (this.totalItems <= this.compOffLogs.length && this.page === 1) return 1;
    return Math.ceil(this.totalItems / this.pageSize) || 1;
  }

  get startItemIndex(): number {
    if (this.compOffLogs.length === 0) return 0;
    return (this.page - 1) * this.pageSize + 1;
  }

  get endItemIndex(): number {
    const computedEnd = this.page * this.pageSize;
    if (this.totalItems <= this.compOffLogs.length && this.page === 1) return this.compOffLogs.length;
  ...
  /**
   * Calculates total comp off days based on date range span, startSession, and endSession parameters
   */
